  ...
  <div class="paging">
    ...
    [#if page_total > 1]
      ...
      <span class="backwards controls">
        <span class="prev control"><a class="val" href="${links.getCMSLink('cms:/pg/-1').url.addParams(url_params).addParam('os', ((page_cur-1)*p_limit)).getURL()}">Prev</a></span>
      </span>
      [/#if]
      ...
      <span class="pages">
        [#if page_total > page_show_limit]
          [#list page_main_start..page_main_end as temp]
            [#if page_cur == temp]
              <span class="page cur[#if temp_index == 0] first[/#if][#if !temp_has_next] last[/#if]"><span class="val">${(temp+1)}</span></span>
            [#else]
              <span class="page control[#if temp_index == 0] first[/#if][#if !temp_has_next] last[/#if]"><a class="val" href="${links.getCMSLink('cms:/pg/-1').url.addParams(url_params).addParam('os', (temp*p_limit))}"><span>${(temp+1)}</span></a></span>
            [/#if]
          [/#list]
        [#else]
      ...
      <span class="forwards controls">
        <span class="next control"><a class="val" href="${links.getCMSLink('cms:/pg/-1').url.addParams(url_params).addParam('os', ((page_cur+1)*p_limit))}">Next</a></span>
      </span>
      [/#if]
    [/#if]
    </div>
...
@param vertical_center OPTIONAL - Boolean on whether to force the image to have a margin offset based on the actual height vs the max height
...
[#function getScaledImage image width height alt="" force_dims=false vertical_center=false include_itemprop=true]
  ...
  [#if image_src?has_content]
    ...
    [/#if]
    [#if force_dims]
      [#if width != 0][#local image_w = width][/#if]
      [#if height != 0][#local image_h = height][/#if]
    [/#if]
    [#local style = ""]
    [#if vertical_center]
    ...
    [/#if]
    [#local image_html = '<img ' + itemprop_html + ' ${style} src="${image_src}" alt="${alt?html}" width="${image_w}" height="${image_h}" />']
  [/#if]
  [#return image_html]
[/#function]
